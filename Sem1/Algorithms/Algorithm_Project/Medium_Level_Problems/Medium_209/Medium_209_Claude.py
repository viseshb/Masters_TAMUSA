...
class Solution:
    def minSubArrayLen(self, target: int, nums: list[int]) -> int:
        ...
        # If the sum of all elements is less than target, return 0
        if prefix_sum[-1] < target:
            return 0
        
        min_length = n + 1  # Initialize to a value larger than possible answers
        
        # For each starting position, binary search for the end position
        for start in range(n):
            # We need to find the first position where sum >= target
            left, right = start, n
            while left < right:
                mid = (left + right) // 2
            ...
            # If we found a valid ending position
            if left < n:
                subarray_sum = prefix_sum[left + 1] - prefix_sum[start]
                if subarray_sum >= target:
                    min_length = min(min_length, left - start + 1)
        
        # If we didn't find any valid subarray
        return min_length if min_length <= n else 0


test_cases = [
    ([1965, 6790, 1765, 1619, 6111, 7031, 3299, 7684, 2293, 5196, 1605, 5314], 1970, 1),
