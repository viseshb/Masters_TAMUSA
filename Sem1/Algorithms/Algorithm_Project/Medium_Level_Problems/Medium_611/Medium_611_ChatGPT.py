...
class Solution:
    def triangleNumber(self, nums: List[int]) -> int:
        # Sort the numbers to make it easier to check the triangle inequality
        nums.sort()
        ans = 0
        n = len(nums)
        
        # Loop through each pair of elements (i, j) where i < j
        for i in range(n - 2):
            for j in range(i + 1, n - 1):
                # Use binary search to find the valid third side
                k = bisect_left(nums, nums[i] + nums[j], lo=j + 1) - 1
                # If k > j, we have found valid triangles for this pair
                if k > j:
                    ans += k - j
